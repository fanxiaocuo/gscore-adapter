...
`token` 请老实写在这个字段里，不要内联成地址的 `?token=` 查询串：只写到 `host:port` 的地址在规范化时只保留 origin，查询串会连同凭据一起丢掉，运行时压根用不上。
...
自定义路径地址里内联的 `?token=` 会被摘出来当作这条连接的凭据（这是它与根地址的另一处不同），面板与出图前一律砍掉查询串再显示。仍然建议写在 `token` 字段里：地址会出现在日志、状态图与面板卡片上。
