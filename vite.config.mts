...
export default defineConfig({
  ...
  define: {
     ...
     * 构建戳，面板页脚显示它。
     *
     * 用途是**分辨手上这一份是不是新包** —— 页脚的时间与你刚构建的时间对不上，就是浏览器
     * 还拿着旧包。
     * 注意：不做 URL 层的防缓存是因为**不需要**，不是因为做不到。宿主那条静态路由是
     * `res.sendFile`，Express 5 默认给 `Cache-Control: public, max-age=0` 加 ETag（实测过），
     * 浏览器每次都回源校验，文件变了就拿新的。先前这里写的是「文件名被白名单钉死、只能靠
     * 硬刷新」，那个前提是错的。
