...
装了 **QQBot-Web-Adapter** 的话，它的控制台左侧会多一页「早柚核心适配器」☄️。页面顶上常驻连接数与今日/累计中转计数，下面分三个 tab：

| tab | 管什么 |
| --- | --- |
| **连接** | 连接的实时状态、增删改与启停、逐账号绑定开关，以及心跳与 ws 总开关 |
| **设置** | 总开关、媒体与外链、内置文件服务、更新检查 |
| **过滤** | 上报开关、触发/屏蔽前缀与关键词、群黑白名单、用户黑名单 |

覆盖面与锅巴一致，另外还多出锅巴没有的 `file_server.*` 与 `upload_hook`。`bot_id_map` 不在面板上（它是映射表，改它请直接编辑 yaml）。页面里用的是插件自己的图标；导航栏那颗只能是 emoji，宿主按纯文本渲染它。

面板不自带服务器，是挂在那个宿主上的插件页 —— 它开机时扫 `webadapter/index.js` 并注册页面与接口，接口由宿主统一加登录鉴权（非内网直接 403）。宿主没装时这部分就是死代码，不影响插件其余功能。

改配置走的是和指令同一条路径，**yaml 里的注释会保留**。几处要知道的：

- **开关即时生效**，数字、文本与名单要按底部那条保存栏；文件服务整节延迟提交（端口和地址是一个意图，分两次写会白重启一次）。
- 媒体与文件上限按 **MB** 填、外链有效期按**秒**填，yaml 里仍存字节与毫秒。没动过的那栏不会因为小数四舍五入被挪几个字节。
- 心跳与 ws 总开关改了会自动重连一次；文件服务的端口或监听地址改了会自动重起，**未取走的外链随之作废**。
- token 与图床凭据只显示「已配置」，不回原值，留空保存表示不改动。
- 群与用户名单可以从已知的群/好友里挑（开弹层时才拉列表），也能直接手填号码。账号离线时列表是空的，**已存的名单仍然生效**。
...
> **配置文件存字节 / 毫秒，三个面板都按 MB / 秒 收。** `media_max_size` / `file_max_size` 在 yaml 里是**字节**，`link_expire` 是**毫秒**；web 面板、锅巴、`#早柚设置最大媒体大小 2` 这三个入口一律按 **MB / 秒** 填，换算在存盘时做（手写的非整数值不会因为四舍五入被挪动）。`#早柚设置 media_max_size=2097152` 是另一种写法，直接收字节 —— 它与 `最大媒体大小 2` 等价。
>
...
| `#早柚设置最大媒体大小 2` | 单位 MB（×1024²），超过这个体积的媒体改用外链，最多 256 |
...
实在不想折腾就调大 `media_max_size` 让图片走 base64 内联，上限 256 MB。
