import { saveConfig, getConnections, enabled } from "../config/index.js";
...
export default class GsCoreAdmin extends plugin {
    ...
    async add(e) {
        ...
        if (!url)
            return e.reply("没解析出地址，用法：#早柚添加连接 ws://127.0.0.1:8765/ws/Yunzai");
        const list = getConnections();
        if (list.some(c => c.url === url))
            return e.reply(`该地址已存在：${url}`);
