import { config, saveConfig, getConnections } from "../config/index.js";
...
export default class GsCoreAdmin extends plugin {
    ...
    async help(e) {
        return e.reply([
            ...
            "#早柚连接列表",
            "",
            "#早柚添加连接 <地址> [name=x] [token=x] [bot_id=x]",
            "  例：#早柚添加连接 ws://127.0.0.1:8765/ws/Yunzai",
            "  例：#早柚添加连接 127.0.0.1:8765 name=主核心 token=abc",
            "  地址只填 host:port 时自动补 /ws/Yunzai",
            "",
            "#早柚删除连接 <名字或序号>",
            "#早柚开启连接 <名字或序号>",
            "#早柚关闭连接 <名字或序号>",
