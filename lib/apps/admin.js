...
import { applyConnections, clients, countSource } from "../modules/client/index.js";
import { expandConnections, requireAccounts, sourceLabel } from "../modules/client/expand.js";
import { readIds } from "../utils/ids.js";
import { DEFAULT_MAX_RECONNECT, MEDIA_SIZE_MAX, STATUS_TEXT, pickByStatus } from "../constants/index.js";
...
import { helpText } from "../modules/render/commands.js";
 ...
 */
function applyListOp(current, ids, op) {
    if (op === "add")
...
export default class GsCoreAdmin extends plugin {
    ...
    async add(e) {
        ...
        if (existing) {
            ...
            if (nextEnable !== (existing.enable !== false))
            ...
            const mapped = nextBind
                ...
                .filter(Boolean);
            return e.reply(`已把账号 ${bind.join("、")} 绑到连接 ${sourceLabel(existing, idx)}\n` +
    ...
    async edit(e) {
        ...
        if (kv.bind !== undefined) {
            ...
            if (kv.bind_op === "add" && !ids.length)
                return e.reply("bind+= 需要填写至少一个账号");
            if (kv.bind_op === "remove" && !ids.length)
                return e.reply("bind-= 需要填写要移除的账号");
            requestedBind = ids;
        ...
        const nextEnable = kv.enable === undefined ? hit.conf.enable !== false : kv.enable.toLowerCase() === "true";
        ...
        const patch = {};
        if (kv.url || nextUrl !== hit.conf.url)
            patch.url = nextUrl;
        if (kv.name)
            patch.name = kv.name;
        if (kv.token !== undefined)
            patch.token = kv.token || null;
        ...
        if (kv.bot_id !== undefined || hit.conf.bot_id)
            patch.bot_id = null;
        ...
        if (kv.bot_id && !ids.length)
            return e.reply("请先 bind=<账号> 再设平台");
        try {
            updateConnection(hit.index, patch, doc => {
                if (kv.bot_id) {
                    for (const id of ids)
                        writeAccountBotId(doc, id, kv.bot_id, undefined, true);
                }
                else {
                    if (hit.conf.bot_id)
                        for (const id of ids)
                    ...
                    if (kv.bind !== undefined)
                        writeAccountBotIds(doc, ids, config.bot_id_map);
                }
            }, !nextEnable
                ? []
