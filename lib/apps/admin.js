import { accountPlatform, config, saveConfig, getWsConnections, appendConnection, updateConnection, removeConnection, enabled, wsEnabled, } from "../config/index.js";
// 注意：只 import 子文件，不碰 @/modules/client barrel —— 那个 barrel 里 framework.ts 有顶层 await 且路径相对 cwd，
// 从云崽根目录跑会给真实 bot 的配置挂上监听
import { applyConnections, countSource } from "../modules/client/lifecycle.js";
import { clients } from "../modules/client/state.js";
import { expandConnections, sourceLabel } from "../modules/client/expand.js";
import { MEDIA_SIZE_MAX, STATUS_TEXT, pickByStatus } from "../constants/index.js";
import { makeLog } from "../utils/compat.js";
import { mergeEndpointQuery, redactUrl, requireWsUrl } from "../utils/url.js";
import { resolveSelfId } from "../utils/message.js";
// 中文设置项的表与解析单独一个模块，理由见 utils/settings.ts 的文件头
import { CN_LABEL, CN_NAMES, doneLine, parseCN } from "../utils/settings.js";
import { renderConfig, renderHelp, renderList, renderSettings } from "../modules/render/pages.js";
import { helpText, rulesFor, stripArg } from "../modules/render/commands.js";
import { botmapWriter, editBotmapWriter, isEnabled, planAdd, planEdit, } from "../modules/connections/plan.js";
...
function toInput(kv, url) {
    return {
        ...
        exclude: kv.exclude !== undefined ? { ids: splitIds(kv.exclude), op: kv.exclude_op } : undefined,
        bot_id: kv.bot_id,
    };
}
/**
...
function sayError(err, from) {
    switch (err.code) {
        ...
        case "list_op_empty":
            return `${err.field}${err.op === "add" ? "+=" : "-="} 需要填写${err.op === "add" ? "至少一个账号" : "要移除的账号"}`;
        case "url_invalid":
            return from === "add"
 ...
 */
function mappedLine(ids, suffix = "") {
    const mapped = ids
...
export default class GsCoreAdmin extends plugin {
    ...
    async show(e) {
        ...
        if (img)
            return e.reply(img);
        return e.reply(`早柚核心适配器  ${enabled() ? "已启用" : "已禁用"}\n` +
    ...
    async add(e) {
        ...
        if (!raw)
            return e.reply("用法：#早柚添加连接 127.0.0.1:8765\n" +
        ...
        if (!plan.ok)
            return e.reply(sayError(plan.errors[0], "add"));
        ...
        if (plan.merge) {
            ...
            try {
                updateConnection(index, patch, botmapWriter(nextBind, requested, explicit), 
            ...
            return e.reply(`已把账号 ${requested.join("、")} 绑到连接 ${sourceLabel(existing, index)}\n` +
                ...
                mappedLine(nextBind) +
                (clientMode()
                    ? `运行时连接：${runningMerge} 条，稍后可用 #早柚状态 查看${runningMerge ? "" : idleReason()}`
                    : "配置已保存"));
        }
        ...
        return e.reply(`已添加连接 ${conf.name}\n核心地址：${redactUrl(conf.url)}\n` +
    ...
    async edit(e) {
        ...
        if (!target || !Object.keys(kv).some(k => CONNECTION_KEYS.includes(k)))
            return e.reply("用法：#早柚修改连接 <名字|序号> bind+=<账号>\n" +
        ...
        if (!hit)
            return e.reply(`找不到连接「${target}」，用 #早柚连接列表 查看`);
        // bind=all 的拒绝留在指令层：它是 key=value 语法的遗留写法，面板没有对应的输入
        if (kv.bind !== undefined && parseBind(kv.bind) === null)
            return e.reply("bind=all 已不再支持：请写明要接入的账号");
        ...
        if (!plan.ok)
            return e.reply(sayError(plan.errors[0], "edit"));
        const { patch, nextBind, nextExclude, nextEnable } = plan.merge;
        const requested = plan.requested || [];
        const explicit = plan.explicit || "";
        try {
            updateConnection(hit.index, patch, editBotmapWriter(nextBind, explicit, hit.conf.bot_id, kv.bind !== undefined), 
            // 停用的连接派生不出运行时连接，那时不能要求「这个账号保存后要连上」；
    ...
    async del(e) {
        ...
        if (!hit)
            return e.reply(`找不到连接「${key}」，用 #早柚连接列表 查看`);
        try {
    ...
    async list(e) {
        ...
        list.forEach((c, i) => {
            ...
            const notes = [];
            if (lead?.status === 1 && unconnected)
                notes.push(`${unconnected} 个账号未连接`);
            if (retry)
                notes.push(`已重连 ${retry} 次`);
            // 注意：后缀只挂在有活客户端那条分支上 —— 停用与未启动的行要的是干净的「已停用」/「未启动」
            const state = c.enable === false
                ? "已停用"
                : !lead
                    ? "未启动"
    ...
    async toggle(e, on) {
        ...
        if (on) {
            if (!clientMode())
                return e.reply(`已启用连接 ${sourceLabel(hit.conf, hit.index)}\n但适配器本体已禁用（enable: false），客户端未运行`);
            ...
            return e.reply(`已启用连接 ${sourceLabel(hit.conf, hit.index)}，但没有可起的运行时连接` +
                (idleReason() || "\n请检查绑定账号（#早柚连接列表 可看）"));
        }
        // 注意：用这次收敛真停掉的条数，而不是停之前 countSource 一把 —— 后者会把「本来就没起来」的也算成断开
        // （停用一条从未连上的连接会回「断开 2 条」）
        const stopped = result.stopped;
        return e.reply(`已停用连接 ${sourceLabel(hit.conf, hit.index)}${stopped ? `，断开 ${stopped} 条` : ""}`);
    }
    async set(e) {
        ...
        if (!Object.keys(kv).length)
            Object.assign(kv, parseCN(raw));
        // 写了参数但两种写法都没解析出来（拼错字段名、忘了开关词）。空参数走 show()，到不了这里
        if (!Object.keys(kv).length)
            return e.reply(`没解析出可设置的项：${raw}\n` +
                `用法：#早柚设置适配器开启\n` +
                `可设：${CN_NAMES}\n` +
                    ...
                    switch (k) {
                        ...
                        case "media_max_size": {
                            ...
                            if (n > MEDIA_SIZE_MAX) {
                                ...
                                errs.push(`最大媒体大小最多 ${MEDIA_SIZE_MAX / 1048576} MB（中文写法单位为 MB），收到 ${v}`);
                                break;
                            }
