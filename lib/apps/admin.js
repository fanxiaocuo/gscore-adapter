import { config, saveConfig, getWsConnections, appendConnection, updateConnection, removeConnection, enabled, } from "../config/index.js";
import { clients, startClient, stopClient } from "../modules/client/index.js";
import { DEFAULT_MAX_RECONNECT, STATUS_TEXT } from "../constants/index.js";
import { makeLog } from "../utils/compat.js";
import { findDuplicate, requireWsUrl } from "../utils/url.js";
...
export default class GsCoreAdmin extends plugin {
    ...
    async add(e) {
        ...
        if (dup) {
            ...
            return e.reply(`这个核心已经加过了：${dup.name}\n` +
                `已绑定：${had}\n` +
                (bind.length
                    ? `本次要绑 ${bind.join("、")}，与它重复。\n` +
        ...
        try {
            ...
            appendConnection(conf, doc => {
                if (mapped)
                    doc.setIn(["bot_id_map", String(selfId)], mapped);
            });
        }
        catch (err) {
    ...
    async edit(e) {
        ...
        const duplicate = findDuplicate(getWsConnections().filter((_, i) => i !== hit.index), nextUrl, nextBind);
        if (duplicate)
        ...
        if (kv.bot_id !== undefined)
            patch.bot_id = kv.bot_id || null;
        ...
        if (next?.enable !== false && clientMode())
            startClient(next);
        const lines = [
        ...
        if (kv.bind !== undefined) {
            ...
            if (finalBotId && nextBind.length > 1)
                lines.push(`注意：本连接的平台标识固定为 ${finalBotId}，bind 里所有账号都按它上报。` +
