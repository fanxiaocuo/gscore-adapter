/**
 * @description 重复上报检测：云崽侧可能同时有多个「往早柚核心上报」的实现，它们互相看不见，同一条消息被上报两次
 * 只告警，不改别人的配置、不禁用别人的功能 —— 用户装了什么是用户的决定，插件越界改动会更难排查。
 * 已知两类来源：ws-plugin 的 `servers[].type == 3`（读运行时的 config/config/ws-config.yaml，不是出厂默认值；
 * type 1/2/6 是 OneBot，不算冲突），以及框架自带的 plugins/adapter/GSUIDCore.js（面向旧版核心，通常收不到东西，
 * 但它注册的 adapter 会让回环判断与账号绑定变复杂）。
...
import { checkFrameworkApis } from "../../utils/compat.js";
/** @description ws-plugin 中代表早柚核心方向的连接类型 */
const GSUID_TYPE = 3;
/** @description 有 gsuid 连接的 ws-plugin 目录名（插件可能被改名，按 ws-config.yaml 存在与否判断） */
...
/** @description 读出某个 ws-config.yaml 里所有早柚方向的连接名 */
...
/** @description 检查并告警。在 client 启动后调用 */
