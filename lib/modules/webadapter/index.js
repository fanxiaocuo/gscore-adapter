 ...
 * 用户写在配置里的注释会被抹掉，这里不学。
 */
import { config, configFile, saveConfig, getWsConnections, enabled } from "../../config/index.js";
import { clients, startClient, stopClient, reloadClients } from "../../modules/client/index.js";
...
import { DEFAULT_MAX_RECONNECT } from "../../constants/index.js";
...
function addConnection(body) {
    ...
    if (dup)
        throw new Error(`这个核心已经加过了（${dup.name}），绑定：${dup.bind?.length ? dup.bind.join("、") : "不限账号"}。改绑其他账号可再加一条`);
    ...
    saveConfig(doc => {
        if (!doc.hasIn(targetPath))
            doc.setIn(targetPath, doc.createNode([]));
        const connections = doc.getIn(targetPath, true);
        if (!isSeq(connections))
            throw new Error("client.ws_connections 应为数组");
        connections.add(doc.createNode(conf));
    });
    if (enabled() && conf.enable)
        startClient(conf);
    return conf.name;
}
/**
...
function editConnection(body) {
    ...
    saveConfig(doc => {
        if (body.url !== undefined)
            doc.setIn([...path, "url"], requireWsUrl(String(body.url)));
        if (body.name !== undefined)
            doc.setIn([...path, "name"], String(body.name).trim());
        if (body.bot_id !== undefined)
            doc.setIn([...path, "bot_id"], String(body.bot_id));
        // token 留空表示「不改」，不是「清空」—— 面板拿不到原值（GET 只回 has_token），
        // 把空串当清空会让每次保存都把 token 抹掉。要清空走 clear_token
        if (body.token)
            doc.setIn([...path, "token"], String(body.token));
        if (body.clear_token)
            doc.setIn([...path, "token"], "");
        if (body.enable !== undefined)
            doc.setIn([...path, "enable"], bool(body.enable, true));
