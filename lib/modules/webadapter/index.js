...
import { findSameCore, inlineToken, mergeEndpointQuery, normalizeEndpoint, redactUrl, requireWsUrl, } from "../../utils/url.js";
...
function editConnection(body) {
    ...
    if (body.name !== undefined) {
        ...
        if (hit.conf.name || submitted !== label(hit.conf))
            patch.name = submitted;
    }
    if (body.bot_id !== undefined || hit.conf.bot_id)
        patch.bot_id = null;
    // 注意：token 留空表示「不改」而不是「清空」—— 面板拿不到原值（GET 只回 has_token），
    // 当清空会让每次保存都把 token 抹掉。要清空走 clear_token
    if (body.token)
        patch.token = String(body.token);
    if (body.clear_token)
        patch.token = "";
    /**
    ...
    if (body.enable !== undefined)
        patch.enable = bool(body.enable, true);
    for (const k of ["reconnect_interval", "max_reconnect_attempts"]) {
        ...
        if (!blank && !Number.isFinite(Number(body[k])))
            throw new Error(`${k} 应为数字`);
        // 注意：间隔为负会让退避算出负延时、setTimeout 立即回调，成热重连循环。次数不设下界，
        // <= 0 就是无限重连
        if (k === "reconnect_interval" && n < 1)
            throw new Error(`${k} 应为不小于 1 的数字`);
        patch[k] = n;
    }
    ...
    const nextBind = (body.bind !== undefined ? patch.bind : hit.conf.bind || []).map(String);
    const nextExclude = (body.exclude !== undefined ? patch.exclude : hit.conf.exclude || []).map(String);
    ...
    updateConnection(hit.index, patch, doc => {
        if (explicit)
            ...
            if (hit.conf.bot_id)
                for (const id of nextBind)
                    writeAccountBotId(doc, id, hit.conf.bot_id);
