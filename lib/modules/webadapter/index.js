 ...
 */
import { accountPlatform, config, configFile, saveConfig, getWsConnections, appendConnection, updateConnection, removeConnection, enabled, withPlatform, profileWithPlatform, } from "../../config/index.js";
    ...
    saveConfig(doc => {
        ...
        if (body.media_max_size !== undefined) {
            ...
            if (!Number.isFinite(n) || n < 1024)
                throw new Error("media_max_size 不能小于 1024");
            // 上限同指令那边：这一栏是「超过就走外链」的阈值，调爆等于关掉外链兜底、每个附件都在内存里 base64
            if (n > MEDIA_SIZE_MAX)
                throw new Error(`media_max_size 不能大于 ${MEDIA_SIZE_MAX}（${MEDIA_SIZE_MAX / 1048576} MiB）`);
        ...
        for (const k of ["heartbeat", "heartbeat_timeout"]) {
            ...
            if (!Number.isFinite(n) || n < 0)
                throw new Error(`${k} 应为 0 或正整数`);
            doc.setIn(["client", k], n);
            changed.push(`client.${k}`);
        }
        const f = typeof body.filter === "object" && body.filter !== null
            ? body.filter
            : {};
...
export function init(ctx) {
    ...
    registerApi("post", "/gscore-adapter/config", guard(req => {
