 ...
 * 用户写在配置里的注释会被抹掉，这里不学。
 */
import { config, configFile, saveConfig, getConnections } from "../../config/index.js";
...
import { requireWsUrl } from "../../utils/url.js";
...
function saveGlobal(body) {
    ...
    saveConfig(doc => {
        if (body.mode !== undefined) {
            ...
            if (!["client", "off"].includes(v))
                throw new Error(`mode 只能是 client 或 off，收到 ${v}`);
            if (v !== config.mode)
                needRestart = true;
            doc.setIn(["mode"], v);
            changed.push("mode");
    ...
    if (touchedClient && config.mode !== "off")
        reloadClients();
...
function addConnection(body) {
    ...
    saveConfig(doc => {
        if (!doc.hasIn(["client", "connections"]))
            doc.setIn(["client", "connections"], doc.createNode([]));
        doc.getIn(["client", "connections"]).add(doc.createNode(conf));
    });
    if (config.mode !== "off" && conf.enable)
        startClient(conf);
    return conf.name;
}
/**
...
function editConnection(body) {
    ...
    saveConfig(doc => {
        if (body.url !== undefined)
            doc.setIn([...path, "url"], requireWsUrl(body.url));
    ...
    if (config.mode !== "off" && next?.enable !== false)
        startClient(next);
