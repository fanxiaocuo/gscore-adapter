 ...
 */
import { accountPlatform, config, configFile, saveConfig, getWsConnections, appendConnection, updateConnection, removeConnection, enabled, wsEnabled, withPlatform, profileWithPlatform, } from "../../config/index.js";
import { UNIT_FIELDS, boundsError, toDisplay, toStored } from "../../config/units.js";
import { applyConnections, clients, reloadClients } from "../../modules/client/index.js";
import { effectiveAccounts, expandConnections, isAutomaticEndpoint, requireAccounts, } from "../../modules/client/expand.js";
import { snapshot, forName } from "../../modules/stats/index.js";
import { passiveCount } from "../../modules/passive/index.js";
import { inlineToken, mergeEndpointQuery, normalizeEndpoint, redactUrl, requireWsUrl, } from "../../utils/url.js";
import { readIds } from "../../utils/ids.js";
import { botmapWriter, editBotmapWriter, planAdd, planEdit, requireSwitch, } from "../../modules/connections/plan.js";
import { writeAccountBotId } from "../../config/botmap.js";
...
import { STATUS_TEXT, pickByStatus, reconnectBase, reconnectCap } from "../../constants/index.js";
 ...
 */
function connView(conf, i, runtime, online) {
    const enabled = conf.enable !== false;
    const views = runtime.map(rt => {
...
function configView() {
    ...
    return {
        enable: enabled(),
        media_max_size: toDisplay("media_max_size", mediaMaxSize()),
        file_max_size: toDisplay("file_max_size", fileMaxSize()),
        link_expire: toDisplay("link_expire", linkExpire()),
        ...
        filter: {
            ...
            prefix: arr(f.prefix),
            block_prefix: arr(f.block_prefix),
            block_include: arr(f.block_include),
            white_group: arr(f.white_group),
            black_group: arr(f.black_group),
            black_user: arr(f.black_user),
        },
        update_check: {
...
function payload() {
    ...
    const connections = list.map((c, i) => connView(c, i, runtime.filter(r => r.sourceIndex === i), online));
 ...
 */
async function checkUploadHook(p) {
    // 空串 = 不启用，直接放过
    if (!p)
        return;
    ...
    try {
        ...
        throw new Error(`加载 upload_hook 失败：${err instanceof Error ? err.message : String(err)}。留空可关掉它`);
    }
...
async function saveGlobal(body) {
    ...
    saveConfig(doc => {
        ...
        for (const field of Object.keys(UNIT_FIELDS)) {
            ...
            if (boundsError(field, next)) {
                ...
                if (fresh !== undefined && !boundsError(field, fresh))
                    next = fresh;
            }
            /*
        ...
        for (const { path: field, label: cn, min, max } of NUM_FIELDS) {
            ...
            if (!Number.isFinite(n) || !Number.isInteger(n))
                throw new Error(`${cn}应为整数`);
            if (n < min)
                throw new Error(`${cn}不能小于 ${min}，收到 ${n}`);
            if (max !== undefined && n > max)
                throw new Error(`${cn}不能大于 ${max}，收到 ${n}`);
        ...
        const clearToken = pick(body, "file_server.imagebed_token_clear") ?? pick(body, "imagebed_token_clear");
        if (next) {
    ...
    if (before.port !== after.port || before.host !== after.host || before.enable !== after.enable) {
        try {
            ...
            if (!r.was)
                notes.push(after.enable
                    ? "已保存；内置文件服务当前没在运行（框架自带文件服务时用不到它），下次真需要外链时按新配置启动"
                    : "已保存；内置文件服务本来就没在运行");
            else if (!after.enable)
                notes.push("内置文件服务已关闭，未取走的外链已作废");
            else if (r.port === null)
                notes.push(`配置已保存，但文件服务没能在 ${after.host}:${after.port || "随机端口"} 上起来（端口可能被占用），` +
                    "大文件外链暂时不可用，详见日志");
            else
                notes.push(`文件服务已按新配置重启（${after.host}:${r.port}），未取走的外链已作废`);
        }
        catch (err) {
            // 配置已经落盘了，重起失败不该把整次保存报成失败 —— 说清楚「配置存了但服务没起来」更有用
            makeLog("error", ["重启内置文件服务失败", err], "GsCore");
            notes.push(`配置已保存，但文件服务没能按新配置起来：${err instanceof Error ? err.message : String(err)}`);
        }
    }
...
function addConnection(body) {
    ...
    if (!plan.ok)
        throw new Error(sayPanelError(plan.errors[0]));
    ...
    if (plan.merge) {
        ...
        updateConnection(index, patch, botmapWriter(nextBind, requested, explicit), nextEnable && requested.length
    ...
    appendConnection(conf, botmapWriter(requested, requested, explicit), conf.enable === false
        ? []
        : requested.length
...
function editConnection(body) {
    ...
    const explicit = plan.explicit || "";
    updateConnection(hit.index, patch, editBotmapWriter(nextBind, explicit, hit.conf.bot_id, body.bind !== undefined), !nextEnable
        ? []
...
function toggleConnection(body) {
    ...
    updateConnection(hit.index, { enable: on }, undefined, on ? [{ sourceIndex: hit.index, action: "启用" }] : []);
...
function bindConnection(body) {
    ...
    // 账号悄悄规范成字符串
    if (freed)
        patch.exclude = exclude.length ? exclude : null;
    updateConnection(hit.index, patch, doc => (on ? writeAccountBotId(doc, id, accountPlatform(id)) : undefined), nextEnable && on ? [{ sourceIndex: hit.index, account: id, action: "绑定" }] : []);
 ...
 *          一按保存就把存着的名单抹平
 */
function targetMap(bot, kind) {
    if (!bot)
        return null;
    try {
        const map = kind === "group" ? (bot.getGroupMap?.() ?? bot.gl) : (bot.getFriendMap?.() ?? bot.fl);
 ...
 */
function targetAvatar(bot, kind, key, info) {
...
function targets(kind) {
    const want = String(kind) === "friend" ? "friend" : "group";
    const online = onlineBots();
    const items = [];
    const seen = new Set();
    const collect = (bot) => {
     ...
     */
    const globalOk = fromAccount ? false : collect(globalThis.Bot);
    /**
    ...
    const note = fromAccount || (globalOk && items.length)
        ? ""
        : online.length
...
export function init(ctx) {
     ...
     */
    registerApi("get", "/gscore-adapter/targets", guard(req => targets(req.query?.kind), 500));
    /**
    ...
    registerApi("post", "/gscore-adapter/config", guard(async (req) => {
    ...
    registerApi("post", "/gscore-adapter/connection", guard(req => {
    ...
    registerApi("post", "/gscore-adapter/reconnect", guard(() => {
