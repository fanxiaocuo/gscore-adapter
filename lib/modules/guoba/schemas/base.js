...
export const baseSchemas = [
    ...
    },
    {
        ...
        label: "媒体内联上限（MiB）",
        bottomHelpMessage: "媒体转 base64 的大小上限，超过则改用 link:// 外链。配置文件里仍按字节存。若早柚核心跑在 Docker 里，需把框架 server.url 配成对端可达的地址",
    ...
    },
    {
        field: "file_max_size",
        label: "文件大小上限（MiB）",
        bottomHelpMessage: "file 段必须内联 base64（协议无 URL 形式），超过此大小直接拒绝发送。配置文件里仍按字节存",
        component: "InputNumber",
        componentProps: { ...SIZE_RANGE, step: 1, precision: 2, addonAfter: "MiB" },
    },
    {
        field: "link_expire",
        label: "外链有效期（秒）",
        bottomHelpMessage: "link:// 外链的有效期。云崽默认只保留 60 秒，核心拉取慢会拿到超时占位图。配置文件里仍按毫秒存",
        component: "InputNumber",
    ...
    },
    {
        ...
        label: "定时检查更新",
        bottomHelpMessage: "定时检查插件仓库有没有新提交，只通知不会自动更新。关掉后 #早柚检查更新 仍可手动用",
        component: "Switch",
    },
    {
