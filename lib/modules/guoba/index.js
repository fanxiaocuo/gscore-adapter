 ...
 */
function splitBlackUser(value) {
    const list = Array.isArray(value) ? value : [];
    const known = friendIds();
    if (!known)
        return { picked: [], manual: list };
    const picked = [];
    const manual = [];
    for (const id of list)
        (known.has(String(id)) ? picked : manual).push(id);
...
function toDisplay(field, value) {
    ...
    if (!scale || typeof value !== "number" || !Number.isFinite(value))
        return value;
 ...
 */
function boundsError(field, stored) {
    const bound = STORED_BOUNDS[field];
    if (!bound || typeof stored !== "number" || !Number.isFinite(stored))
        return null;
    const divisor = SCALED_FIELDS[field]?.divisor ?? 1;
    const got = Math.round((stored / divisor) * 100) / 100;
    if (stored < bound.min)
        return `${bound.label}至少 ${bound.minText}，收到 ${got} ${bound.unit}`;
    ...
    return {
        ...
        configInfo: {
            ...
            setConfigData(data, { Result }) {
                ...
                try {
                    const connectionsTouched = fields["client.connections"] !== undefined &&
