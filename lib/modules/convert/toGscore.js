 ...
 * @description 云崽 -> 早柚核心
 */
import { toGscoreMedia, toGscoreFile, isChannel, resolveReplyId, resolveReplyMessage, resolveForwardMessage, hasReplyContext, } from "../../utils/index.js";
import { buttonsToGscore } from "./buttons.js";
import { makeLog, toStr } from "../../utils/compat.js";
...
function replyTextValue(value) {
    ...
    try {
        ...
        if (parsed?.type === "markdown" && parsed?.data?.content)
            return String(parsed.data.content);
    }
    catch {
        // 普通文本不是 JSON，按原样返回。
    }
...
function replyText(message) {
    ...
    for (const item of list) {
        ...
        if (item.type === "markdown" || item.type === "json") {
            ...
            const value = typeof data === "object" && data !== null ? (data.content ?? data.text ?? data.data) : data;
 ...
 */
async function flattenNodes(nodes) {
    const arr = [];
 ...
 */
export async function msgToGscore(msg, e) {
    ...
    for (const i of list) {
        ...
        switch (i.type) {
            ...
            case "reply": {
                ...
                if (id != null && id !== "")
                    out.push({ type: "reply_id", data: String(id) });
                break;
            }
            case "button":
...
export async function yunzaiToGscore(e, botId, opts = {}) {
    ...
    if (hasReplyContext(e)) {
        ...
        if (quotedMessage.length) {
            ...
            if (quotedText)
                content.push({ type: "reply", data: quotedText });
