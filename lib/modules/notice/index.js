/**
 * @description 非消息事件（meta event）转换
 * 核心按 `meta-` 前缀识别段类型，data 为 dict 时整体存入 event.meta_event_data，并用其中的 user_id / group_id
 * 回填顶层缺失字段供权限与黑白名单使用 —— 所以必需字段缺失时宁可整包丢弃，不发残包。
 */
import { SUB_TYPE_MAP } from "../../constants/index.js";
import { str, passFilter, isChannel } from "../../utils/index.js";
import { toStr } from "../../utils/compat.js";
export { passFilter };
/**
 * @description 云崽 notice 事件 -> meta event
 * @param selfId 调用方解析过的账号，用于 poke 的被戳者兜底；不传则退回 e.self_id
 * @returns 无法映射返回 null
 */
export function noticeToMeta(e, selfId) {
    if (!e || e.post_type !== "notice")
        return null;
    // 注意：poke 判断必须在 group/friend 之前 —— 它在两种 notice_type 下都出现
    if (e.sub_type === "poke") {
        // OneBotv11 把 operator_id 赋为戳人者
    ...
    // 注意：这里没有 sub_type —— OneBot 原生的 approve/invite/kick/leave 已被 OneBotv11 用拆出来的
    // increase/decrease 覆盖，取不回来了
    return { eventName, data };
}
/**
 * @description meta event -> 完整 MessageReceive
 * @param e     云崽 notice 事件
 * @param meta  noticeToMeta 的产物
 * @param botId 平台标识（resolveBotId 的结果）
 * @param opts  { isMaster, selfId }
 */
export function metaToGscore(e, meta, botId, opts = {}) {
    if (!meta)
        return null;
    ...
    return {
        ...
        msg_id: "",
        // 注意：频道要判在前 —— QQBot-Plugin 的频道事件 group_id 带 qg_ 前缀但 message_type 是 group，
        // 只看 group_id 有无会把频道事件标成 group（同 toGscore 的处理）
