...
export function Status(data) {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs(Page, { palette: p, word: data.ghost, children: [_jsx(Header, { title: data.heading, status: "GSCORE_ADAPTER", led: data.enabled ? "on" : "off", rightKey: "ADAPTER", rightValue: data.enabled ? "ENABLED" : "DISABLED" }), _jsx(Stats, { items: data.summary, palette: p }), data.rows.length === 0 ? (_jsx(Empty, { title: "\u6682\u65E0\u8FDE\u63A5", tip: data.emptyTip || "用 #早柚添加连接 <地址> 添加" })) : (_jsx("div", { className: "flex flex-col gap-[22px]", children: data.rows.map(row => {
                    ...
                    _jsx("div", { className: "mt-[72px] grid [grid-template-columns:repeat(2,1fr)] gap-[56px_64px]", children: data.panels.map((panel, pi) => (
                        ...
                        _jsxs("div", { className: "min-w-0", children: [_jsx(Section, { title: panel.title, color: p.rotate[pi % p.rotate.length], right: panel.key }), _jsx("div", { className: "flex flex-col gap-[14px]", children: panel.items.map((it, ii) => (
                                    ...
                                    _jsxs("div", { className: "flex items-baseline gap-[14px] text-[23px] leading-[1.5]", children: [_jsx("span", { className: "flex-none text-muted", children: it.k }), _jsx("span", { className: "min-w-0 flex-1 break-words text-right font-mono font-bold", children: it.v })] }, ii))) })] }, pi))) }))] }), _jsx(Footer, { name: data.title, version: data.version, palette: p, lines: [data.time, "#早柚帮助 查看全部指令"] })] }));
}
