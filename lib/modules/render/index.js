...
export async function render(opts) {
    ...
    const data = {
        ...
        imgType: "jpeg",
        /*
         * 82 而非 88：压花玻璃之后体积翻了几倍，这是唯一不动设计的压缩旋钮
         *
         * 换成压花玻璃之后实测（生产的 scale 1.25，1800×3565）：
         *   帮助页 COOL 2409KB / LIGHT 1419KB，而把高光层整个关掉只有 527KB
         * —— 也就是说**这层纹理占了体积的 78%**。它是这版设计的主体，不能删。
         *
         * 剩下的旋钮里 quality 是唯一不改观感的：q88→82 少 22%，而 JPEG 在高频纹理上
         * 的块效应正好被纹理本身盖住（纯色大面积才显块）。各档实测：
         *   q88 2409KB   q82 1877KB   q76 1533KB   q70 1320KB
         * 停在 82：76 起纹理的细鳞片开始被抹平，那是在削这版设计要的东西。
         *
         * 另外两个旋钮记在这里，别再重新试一遍：
         *   gloss opacity  .85→.45 能到 1715KB，但纹理明显发闷（见 theme.ts 的取值理由）
         *   SCALE 1.25→1.0 能到 1627KB，代价是 QQ 里放大看文字会糊
         *
         * 耗时与 quality 无关：各档都在 680~700ms（同一台机器、每次新开 page 量的）。
         */
        quality: 82,
