...
const GROUP_ORDER = ["状态与连接", "连接管理", "全局设置", "更新"];
 ...
 */
export function stripArg(app, fnc, msg) {
    const c = COMMANDS.find(x => x.app === app && x.fnc === fnc);
    if (!c)
        throw new Error(`未登记的指令：${app}#${fnc}`);
...
/** @description 派生帮助图分组 */
export const HELP_GROUPS = GROUP_ORDER.map(title => ({
    title,
