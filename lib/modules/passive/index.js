 ...
 * 只可能是 QQ 单聊，4 精确对得上。
 */
const MAX_USES = { direct: 4, group: 5 };
 ...
 */
function usesFor(targetType, id) {
...
export function take(selfId, targetType, targetId) {
    ...
    if (hit.used >= usesFor(targetType, hit.id))
        return "";
    hit.used += 1;
    dirty.add(key);
    return hit.id;
...
async function flush() {
    if (!dirty.size && !pendingRemove.size)
        return;
    const rows = [];
    const gone = new Set();
...
export async function initPassive() {
    ...
    try {
        ...
        for (const r of rows) {
            ...
            if (used >= MAX_USES[typeOfKey(r.key)])
                continue;
...
export function passiveCount() {
    ...
    for (const [key, v] of recent)
        if (now - v.at <= WINDOW_MS && v.used < MAX_USES[typeOfKey(key)])
            n += 1;
    return n;
}
/** 是否在落盘 */
