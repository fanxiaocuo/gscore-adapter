...
import { zero, add, today } from "./counters.js";
...
function pendingRows() {
    ...
    for (const name of dirty) {
        ...
        if (c)
            rows.push({ day: dailyDay, name, ...c });
    }
    return rows;
}
/** 记一次收发。同步，热路径 */
export function count(kind, name) {
    rollover();
    total[kind]++;
    daily[kind]++;
    const key = name || "";
...
export async function initStats() {
    ...
    try {
        ...
        const d = today();
        for (const r of rows) {
            const c = { up: r.up, event: r.event, down: r.down };
            add(total, c);
            if (r.name) {
        ...
        dailyDay = d;
        if (rows.length)
            makeLog("debug", `中转计数已载入：${rows.length} 行，累计上行 ${total.up + total.event}、下行 ${total.down}`, "GsCore");
    }
    catch (err) {
