import { config, configFile } from "../../config/index.js";
import { USER_TYPES } from "../../constants/index.js";
import { logStr } from "../../utils/index.js";
import { msgToGscore, gscoreToYunzai, normalizeEventMsg } from "../../modules/convert/index.js";
...
export class GsCoreServerAdapter {
    ...
    async handleControl(json, bot) {
        ...
        if (seg?.type === "excute_delete_message") {
            ...
            try {
                ...
                const target = json.target_type === "direct"
                ...
                if (!fn)
                    return Bot.makeLog("warn", "当前适配器不支持撤回消息", this.id), true;
        ...
        if (seg?.type === "excute_ban_user") {
            ...
            try {
                ...
                if (!group?.muteMember)
                    return Bot.makeLog("warn", "当前适配器不支持禁言", this.id), true;
                await group.muteMember(Number(d.user_id) || d.user_id, duration);
                Bot.makeLog("info", `${duration ? `禁言 ${duration}s` : "解除禁言"}：${d.user_id}@${d.group_id}`, this.id);
            }
            catch (err) {
    ...
    async sendMsg(data, target_type, target_id, msg) {
        ...
        Bot.makeLog("info", `发送${target_type === "direct" ? "好友" : "群"}消息：${logStr(content)}`, `${data.self_id} => ${target_id}`, true);
        data.bot.sendApi({
    ...
    sendGroupMsg(data, msg) {
        ...
        if (USER_TYPES.includes(head))
            return this.sendMsg(data, head, gid.slice(idx + 1), msg);
    ...
    async message(raw, ws) {
        ...
        try {
            ...
            if (await this.handleControl(json, data.bot))
                return;
            ...
            if (logOnly || !message.length)
                return this.sendRecallReceipt(data.bot, json, null);
            if (quote)
            ...
            if (Bot[json.bot_self_id])
                this.sendRecallReceipt(Bot[json.bot_self_id], json, null);
