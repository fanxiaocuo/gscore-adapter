...
export class GsCoreClient {
    ...
    async sendReceive(e, isMaster) {
        ...
        this.send(data);
        Bot.makeLog("debug", `上报早柚核心：${logStr(data.content)}`, `${e.self_id} => ${this.name}`, true);
        return true;
    }
    /**
     * 上行：非消息事件（入群/退群/戳一戳）
    ...
    async onMessage(raw) {
        ...
        try {
            ...
            if (logOnly || !message.length)
                return this.sendRecallReceipt(data, null);
