...
function warnSelfLoop(conf) {
    ...
    if (config.mode === "both" &&
        ...
        Bot.makeLog("warn", `连接 ${conf.name} 的地址指向本机 ${localPort} 端口，both 模式下可能造成消息死循环`, "GsCore");
}
/** 启动单个连接（已存在同名则跳过） */
...
export function startClients() {
    ...
    for (const conf of config.client?.connections || [])
        startClient(conf);
    if (clients.length)
        Bot.makeLog("mark", `早柚核心客户端启动 ${clients.length} 个连接`, "GsCore");
    else
        Bot.makeLog("warn", "早柚核心客户端没有可用连接", "GsCore");
}
