...
if (mode === "server" || mode === "both") {
    Bot.makeLog("warn", `mode: ${mode} 已废弃（早柚核心不会主动连接云崽），已按 client 运行。请把配置改为 mode: client`, "GsCore");
    mode = "client";
}
...
if (mode === "client")
    Bot.once("online", () => startClients());
if (mode === "client")
    Bot.makeLog("info", "早柚核心适配器已载入", "GsCore");
else
    Bot.makeLog("warn", "早柚核心适配器已禁用（mode: off）", "GsCore");
