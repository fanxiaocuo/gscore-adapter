...
function resolveBotId(e, conf) {
	...
	return map[e.bot?.adapter?.id] || map[e.adapter_id] || map[e.bot?.adapter?.name] || map[e.adapter_name] || map.default || "onebot";
}
...
* 它们同属群聊语境，没必要再分一个开关。
*/
function passDirection(e) {
	const f = config.filter || {};
	if (e?.message_type === "group" || e?.isGroup || isChannel(e) || e?.group_id != null) return f.report_group !== false;
...
function fromIcqqSource(e) {
	...
	try {
		...
		if (gid && typeof icqq.genGroupMessageId === "function") return String(icqq.genGroupMessageId(gid, Number(src.user_id) || 0, seq, rand, time, 1));
...
* @returns 没有引用返回空串
*/
function resolveReplyId(e) {
	if (e?.source?.message_id != null && e.source.message_id !== "") return String(e.source.message_id);
...
function sendError(ret) {
	...
	if (ret.error) {
		...
		} else return String(ret.error?.message || ret.error);
	}
	if (ret.status === "failed") return String(ret.msg || ret.wording || "status: failed");
	if (ret.retcode != null && Number(ret.retcode) !== 0) return String(ret.msg || ret.wording || `retcode: ${ret.retcode}`);
	return "";
}
/**
...
async function yunzaiToGscore(e, botId, opts = {}) {
	...
	if (replyId && !e.message?.some?.((i) => i?.type === "reply")) content.push({
		type: "reply",
		data: replyId
	});
...
const dbDir$1 = path.join(PluginPath, "data");
const dbFile$1 = process.env.GSCORE_STATS_DB || path.join(dbDir$1, "stats.db");
...
*/
let chain$1 = Promise.resolve();
function queue$1(fn) {
	const next = chain$1.then(fn, fn);
	chain$1 = next.catch(() => {});
	return next;
}
/** run/all 的 Promise 包装。sqlite3 是 callback API，没有原生 Promise 接口 */
function run$1(sql, params = []) {
	return new Promise((resolve, reject) => {
		db$1.run(sql, params, (err) => err ? reject(err) : resolve());
	});
}
function all$1(sql, params = []) {
	return new Promise((resolve, reject) => {
		db$1.all(sql, params, (err, rows) => err ? reject(err) : resolve(rows || []));
	});
}
/**
* 打开数据库并建表
...
async function open$1() {
	...
	try {
		fs.mkdirSync(dbDir$1, { recursive: true });
		db$1 = await new Promise((resolve, reject) => {
			const d = new sqlite3.Database(dbFile$1, (err) => err ? reject(err) : resolve(d));
		});
		await run$1("PRAGMA journal_mode = WAL");
		await run$1("PRAGMA synchronous = NORMAL");
		await run$1(`CREATE TABLE IF NOT EXISTS relay (
      day   TEXT    NOT NULL,
      name  TEXT    NOT NULL,
      ...
      PRIMARY KEY (day, name)
		...
		await run$1("CREATE INDEX IF NOT EXISTS idx_relay_name ON relay (name)");
		await run$1(`CREATE TABLE IF NOT EXISTS meta (
...
function metaSince(fallback) {
	if (!db$1) return Promise.resolve(fallback);
	return queue$1(async () => {
		if (!db$1) return fallback;
		const rows = await all$1("SELECT value FROM meta WHERE key = 'since'");
		const got = Number(rows[0]?.value);
		if (got > 0) return got;
		await run$1("INSERT OR REPLACE INTO meta (key, value) VALUES ('since', ?)", [String(fallback)]);
		return fallback;
	});
}
/** 读全部明细，用于启动时灌入内存 */
function load$1() {
	if (!db$1) return Promise.resolve([]);
	return all$1("SELECT day, name, up, event, down FROM relay");
}
/**
* 回写若干 (day, name) 的**绝对值**
*
...
*/
function save$1(rows) {
	if (!db$1 || !rows.length) return Promise.resolve();
	...
	return queue$1(async () => {
		...
		await run$1("BEGIN");
		try {
			for (const r of rows) await run$1(sql, [
				r.day,
				r.name,
				r.up,
				r.event,
...
function clear() {
	...
	return queue$1(async () => {
		if (!db$1) return;
		await run$1("DELETE FROM relay");
		await run$1("DELETE FROM meta WHERE key = 'since'");
	});
}
/** 数据库是否可用 */
function available() {
...
function rollover() {
	...
	todayRows.clear();
	dirty$1.clear();
	if (pending.length) lastRollover = save$1(pending).catch((err) => makeLog("debug", ["中转计数回写失败", err], "GsCore"));
}
/** 最近一次跨日回写，供退出时等待 */
...
async function flush$1() {
	if (!dirty$1.size) return;
	...
	try {
		...
		for (const r of rows) dirty$1.add(r.name);
	}
}
/**
...
async function initStats() {
	if (!await open$1()) return;
	try {
		const rows = await load$1();
		const d = today();
		for (const r of rows) {
			const c = {
				up: r.up,
	...
	timer$1 = setInterval(flush$1, FLUSH_MS$1);
	timer$1.unref?.();
...
async function stopStats() {
	if (timer$1) clearInterval(timer$1);
	timer$1 = null;
	await lastRollover;
	await flush$1();
	await close$1();
...
//#endregion
//#region src/modules/passive/db.ts
/**
* QQBot 被动回复：最近入站消息 id 的落盘层
*
* 为什么落盘
* ---------
* 被动回复窗口是 5 分钟。不落盘的话云崽一重启，这 5 分钟内所有下发都退回主动推送，
* 白烧额度。数据量极小（每个会话一行，只留最新一条），代价可以忽略。
*
* 为什么是 sqlite 而不是 redis
* --------------------------
...
* 只写内存，按脏标记定时回写。掉电最多丢最后几秒的 id，那只意味着少省几次额度。
*/
...
function load(minAt) {
	...
	return all("SELECT key, id, at FROM passive WHERE at >= ?", [minAt]);
}
/** 回写若干行（绝对值，幂等） */
function save(rows) {
	if (!db || !rows.length) return Promise.resolve();
	const sql = `INSERT INTO passive (key, id, at) VALUES (?, ?, ?)
    ON CONFLICT (key) DO UPDATE SET id = excluded.id, at = excluded.at`;
	return queue(async () => {
		if (!db) return;
		await run("BEGIN");
		try {
...
//#endregion
//#region src/modules/passive/index.ts
/**
* QQBot 被动回复
*
* 问题
...
function evict() {
	const now = Date.now();
	for (const [k, v] of recent) if (now - v.at > WINDOW_MS) recent.delete(k);
	if (recent.size <= MAX) return;
	const sorted = [...recent.entries()].sort((a, b) => a[1].at - b[1].at);
	for (const [k] of sorted.slice(0, recent.size - MAX)) recent.delete(k);
}
/**
* 取一个可用于被动回复的 id，**取出即删**
*
...
*/
function take(selfId, targetType, targetId) {
...
async function flush() {
	...
	dirty.clear();
	try {
		if (rows.length) await save(rows);
...
var GsCoreClient = class {
	...
	*/
	passiveSender(adapter, type, targetId) {
		...
		const fn = adapter[type === "direct" ? "sendFriendMsg" : isGuild ? "sendGuildMsg" : "sendGroupMsg"];
