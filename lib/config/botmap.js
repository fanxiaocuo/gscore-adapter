 ...
 */
function mapKey(doc, sid) {
    if (doc.hasIn(["bot_id_map", sid]))
        return sid;
    const n = Number(sid);
    if (/^\d+$/.test(sid) && String(n) === sid && doc.hasIn(["bot_id_map", n]))
        return n;
    return sid;
}
function mapValue(doc, sid) {
 ...
 */
export function writeAccountBotId(doc, selfId, platform, existingMap, force = false) {
    const sid = String(selfId ?? "").trim();
    if (!sid)
        return "";
    if (!force && existingMap?.[sid])
        return "";
    if (!force && mapValue(doc, sid))
        return "";
...
export function writeAccountBotIds(doc, ids, existingMap) {
    const written = [];
    for (const id of ids) {
...
export function readIdList(item, key) {
    const node = item.get(key, true);
    if (!YAML.isSeq(node))
        return [];
    return [
        ...new Set(node.items.map(n => (YAML.isScalar(n) ? String(n.value ?? "").trim() : "")).filter(Boolean)),
    ];
}
/**
 ...
 */
export function syncConnectionAccounts(doc, existingMap) {
