...
import { ConfigPath, PluginPath } from "../dir.js";
...
function migrateMode(doc) {
    ...
    const pair = pairOf("enable");
    if (pair?.key && typeof pair.key === "object" && !pair.key.commentBefore) {
        const mine = " 是否启用适配器。false 则完全不连早柚核心（取代旧版的 mode: client/off）\n" +
            " 改这项需重启云崽生效";
...
export function upgradeUserConfig(userFile) {
    ...
    if (userDoc.errors?.length || defDoc.errors?.length)
        return changes;
