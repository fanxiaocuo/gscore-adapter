...
export function isAutoYunzaiPath(pathname) {
 ...
 */
export function mergeEndpointQuery(prev, next) {
    ...
    for (const name of new Set(source.searchParams.keys())) {
        ...
        if (target.searchParams.has(name))
            continue;
        for (const value of source.searchParams.getAll(name))
 ...
 */
export function inlineToken(url) {
    try {
 ...
 */
export function materializeAccountUrl(endpoint, account) {
    const id = encodeURIComponent(String(account).trim());
    if (!id)
        throw new Error("绑定账号不能为空");
    const u = new URL(endpoint);
    if (u.pathname !== "/" && u.pathname !== "")
        throw new Error("自定义路径不能生成账号连接地址");
 ...
 */
export function findSameCore(list, url) {
    const target = coreKey(url);
    if (!target)
        return undefined;
    let newPath = "";
...
export function requireWsUrl(url) {
    ...
    if (u.protocol === "http:" || u.protocol === "https:") {
        ...
        throw new Error(`不支持 http://，早柚核心只能用 WebSocket 连。请改用：${suggest}\n` +
