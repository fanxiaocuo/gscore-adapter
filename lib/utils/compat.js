 ...
 */
export function makeLog(level, msg, tag, force = false) {
    const B = globalThis.Bot;
    if (typeof B?.makeLog === "function")
        return B.makeLog(level, msg, tag, force);
 ...
 */
export async function toBuffer(file, opts = {}) {
    const B = globalThis.Bot;
    if (typeof B?.Buffer === "function")
        return B.Buffer(file, opts);
    ...
    if (s.startsWith("base64://"))
        return Buffer.from(s.slice(9), "base64");
    ...
    if (/^https?:\/\//.test(s)) {
        ...
        try {
            ...
            if (opts.size && buf.length > opts.size)
                return s;
            return buf;
        }
        catch {
 ...
 */
export function fileToUrl(file, opts) {
    const B = globalThis.Bot;
    if (typeof B?.fileToUrl === "function")
        return B.fileToUrl(file, opts);
 ...
 */
export async function makeForwardMsg(nodes, target) {
    const B = globalThis.Bot;
    // TRSS：同步返回标记对象。用 try 包住是因为 Miao 继承来的那个会同步抛。
    try {
        const r = B?.makeForwardMsg?.(nodes);
        if (r && typeof r === "object" && !(r instanceof Promise) && r.type === "node")
            return r;
...
const REQUIRED = ["makeLog", "String", "Buffer", "fileToUrl"];
/**
 * @description 返回缺失的 Bot 方法名，空数组表示框架能力齐全
 * 供启动自检使用 —— 静默降级最难排查，缺什么要明说。
 */
export function missingBotApis() {
    const B = globalThis.Bot;
 ...
 */
export function checkFrameworkApis() {
    ...
    if (missing.includes("fileToUrl"))
        makeLog("warn", "当前框架不提供 Bot.fileToUrl（文件外链服务）：" +
            "超过 media_max_size 的大图/大文件将无法上报，小文件走 base64 不受影响。", "GsCore");
}
