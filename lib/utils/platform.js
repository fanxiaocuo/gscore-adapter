 ...
 * 漏认则会静默按 onebot 上报。
 */
export function isQQBotAppId(id) {
    const s = id == null ? "" : String(id);
    if (!s)
        return false;
    if (/^(2854|3889|401)/.test(s) && s.length === 10)
        return true;
 ...
 * @returns 平台标识，推不出返回 ""
 */
export function guessPlatform(selfId, bot) {
    ...
    if (adapterId === "QQBot" || isQQBotAppId(sid))
        return "qqgroup";
