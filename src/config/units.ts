...
/**
 * @description 一栏的换算与边界
 * divisor 是整数除数而非小数位数（MB 是 1048576，写成位数会差 4.8%）
 */
...
export function toDisplay(field: string, value: unknown): unknown {
  const f = UNIT_FIELDS[field]
  if (!f || typeof value !== "number" || !Number.isFinite(value)) return value
...
export function boundsError(field: string, stored: unknown): string | null {
  const f = UNIT_FIELDS[field]
  if (!f || typeof stored !== "number" || !Number.isFinite(stored)) return null
  const got = round2(stored / f.divisor)
  if (stored < f.min) return `${f.label}至少 ${f.minText}，收到 ${got} ${f.unit}`
