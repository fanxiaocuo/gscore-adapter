 ...
 * 连接状态数值的可读名
 *
 * 显式标 Record 而不是让它推断：渲染那两处（pages.ts 的主行与子行 state）拿
 * `0 | 1 | 2 | 3` 直接索引，标了之后「查表必有值」由类型保证，那边就不用挂
 * `|| String(status)` 这种永远走不到的兜底。将来真加了状态码 4，漏配文案会在
 * 索引处编译报错 —— 比运行时显示一个裸数字更早、更明确。
 *
 * 与 {@link statusRank} 的差别正在这里：那个收 `number`，得容忍表外的码。
 */
