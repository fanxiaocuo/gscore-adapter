...
const NOTE =
  "flex-none rounded-[999px] border border-border px-[8px] py-[1px] text-[11px] text-muted"
 ...
 */
const ROW =
  "grid min-h-[66px] grid-cols-[auto_minmax(120px,1fr)_minmax(90px,0.65fr)_auto] items-center gap-x-[12px] gap-y-[2px] border-t border-border px-[12px] first:border-t-0 max-[720px]:min-h-[67px] max-[720px]:grid-cols-[auto_minmax(0,1fr)_auto]"
...
export function BotSwitchList({
   ...
   * 灰着 + 一句悬停说明，比让用户先失败一次再读报错好。
   */
  const locked = automatic && checked.length === 1

  return (
    <div className="mt-[10px] overflow-hidden rounded-[10px] border border-border bg-bg">
      {bots.length === 0 ? (
        <p className="p-[12px] text-[12px] text-muted">
          {empty || "没有可选账号：当前没有机器人在线，这条连接也没绑过账号。"}
        </p>
      ) : (
        bots.map(b => {
