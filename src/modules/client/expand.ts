/**
 * @description 逻辑连接展开成运行时连接：根端点按有效账号逐条派生，自定义与旧路径只起一条兼容连接
 * 核心侧以 `/ws/{bot_id}` 认客户端，不按账号展开就会让多个机器人共用路径、互相顶替。
 */
...
/**
 * @description 展开时跳过或降级一条连接的原因，带上是哪一条出的问题
 * 带 sourceIndex 是让调用点只打属于本次改动的那几条；冲突记在被跳过那条的名下，它才是起不来的那个。
 */
export interface ExpandError {
   ...
   */
  skipped: boolean
  /** 给人看的原因。只含连接名、来源序号与 pathname，不含完整地址 —— 它会进日志也会回前端 */
  message: string
}
