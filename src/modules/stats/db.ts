...
export function metaSince(fallback: number): Promise<number> {
  ...
  return queue(async () => {
    ...
    const rows = await all<{ value: string }>("SELECT value FROM meta WHERE key = 'since'")
...
export function load(): Promise<RelayRow[]> {
  ...
  return all<RelayRow>("SELECT day, name, up, event, down FROM relay")
}
...
export function save(rows: RelayRow[]): Promise<void> {
  ...
  return queue(async () => {
    ...
    await run("BEGIN")
    try {
      for (const r of rows) await run(sql, [r.day, r.name, r.up, r.event, r.down])
      await run("COMMIT")
    } catch (err) {
