 ...
 */
import { config, getWsConnections, enabled, profileWithPlatform } from "@/config"
 ...
 * 查的是名字而不问客户端还在不在：clients 会被 reloadClients 整个重建，而计数按名字存在模块级，活得比客户端长。
 */
...
function collect(detail = false) {
  ...
  const rows: ConnRow[] = list.map((c, i) => {
    ...
    // 反过来从 clients 里筛而不是逐条 find：展开出来的连接与活着的客户端未必一一对应（reloadClients 会整个重建
    ...
    if (meta.length === 0 && !accounts) {
      ...
      // 指令名是「重连」。原先这里印的是 #早柚重载 —— 那条指令不存在（apps/status.ts 的规则是
      // `^#?早柚(核心)?重连$`），出图上印一条用户照抄会没反应的指令比不印更糟
      else if (!live.length) meta.push("尚未建立连接，可用 #早柚重连 重试")
