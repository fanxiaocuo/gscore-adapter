 ...
 * 跨页共用组件
 *
 * 收录标准：**两个以上页面真的在用**。只有一个页面用的一律放回 pages/ 下，
 * 否则这里会慢慢变成第二个大杂烩。
 *
 * 当前住户与用户：
 *   .stats/.stat  帮助页、状态页、更新日志页的概览统计条
 *   .sec          关于页的分节标题 + 状态页的分组明细标题
 *   .pill         状态页的连接状态胶囊（关于页的 .rt-badge 形近但字号不同，各自留着）
 *   .empty        状态页「暂无连接」、更新日志页「已是最新」
 *   .notice       更新日志页的 fetch 失败提示（预留给其它页）
 */
import type { Palette } from "../theme.js"

export const shared = (p: Palette): string => `
/* ---------- 概览统计条 ---------- */
