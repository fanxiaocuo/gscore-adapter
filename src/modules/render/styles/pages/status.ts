/**
 * 状态页 / 连接列表页（#早柚状态、#早柚连接列表）
 *
 * 组件 components/Status.tsx。结构：概览统计条（shared）→ 逐行连接卡 → 分组明细。
 *
 * 类名 st- 前缀。分节标题用 shared 的 .sec —— 拆分前它借的是关于页私有的
 * .rt-sec，现在两页共用的东西已经搬到 shared.ts，不再有「借用」关系。
 */
import type { Palette } from "../../theme.js"

export const status = (p: Palette): string => `
/* ---------- 连接列表 ---------- */
