 ...
 * 帮助页（#早柚帮助）
 *
 * 组件 components/Help.tsx。结构：分组标题（色条 + 巨型标题）→ 双栏指令卡 → 子分组。
 *
 * 类名一律 hp- 前缀
 * ----------------
 * 拆分前这页用的是 .group / .item / .items / .sub 这类通用名，其中 .items 和
 * 关于页「本版变更」的列表撞名、.sub 又和页脚的小字条撞名，改任何一处都要先
 * 确认另外两个页面没在用。加了前缀之后，这个文件里的规则只可能作用于帮助页。
 * 卡片内部的 .ico/.body/.cmd/.dsc/.eg 保持短名——它们始终被 .hp-item 限定，
 * 不会泄漏出去。
 */
import type { Palette } from "../../theme.js"

export const help = (p: Palette): string => `
/* ---------- 分组 ---------- */
