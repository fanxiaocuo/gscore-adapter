/**
 * @description 运行环境页（#早柚版本）：一张「本机诊断快照」—— 巨大的版本号当主视觉，下面按两列铺环境摘要
 * 版式与取材照 kkk 的 #kkk版本。刻意不沿用状态页那套骨架（Header + 统计条 + 卡片列表）—— 两条命令答的是两个
 * 问题：#早柚状态 答「连接好不好」（运行时的、随时会变），#早柚版本 答「跑在什么上面」（环境的、基本不变），
...
export interface AboutData {
   ...
   * 标题右侧的速览格，不给则标题独占整宽
   *
   * 补的是标题区右侧那片空白（标题只有四个字，88px 下占不到三分之一宽）。内容要选「一眼就想知道」的三两项，
   * 多了会和下面的环境摘要重复。
   */
   ...
   *
   * 与 #早柚更新日志 的分工：那条命令列 git 提交答「代码更新到哪了」，这里列发布条目答「这个版本改了什么」。
   */
...
/**
 * @description 版本号可用宽度：1296（画布减 page padding）再减去 hero 图标 200px 与 44px 间距
 * 注意：几何对应下面 hero 那一块的 size-[200px] 与 gap-[44px]，改那边要同步改这里。
 */
...
export function About(data: AboutData) {
   ...
   * 版本号字号：按串长反推，保证一行放得下
   *
   * 130px 是给 `v2.1.0` 这类短串的理想值，但 main 分支上 git describe 会给出 `v2.1.0-2-gc6522ee-dirty`
   * （130px 下宽约 1790px，远超可用的 1052px），折成两行会破坏右侧那块的层次、第二行还会压到插件名上。
   * 下限 56px 仍比正文的 38px 大一档，主视觉地位保得住。字距 -.05em 与 CSS 一致，长串下能省约 4% 宽度，不能漏算。
   */
  ...
  return (
    <>
      ...
      <div className="relative z-10 p-[72px]">
        ...
        {/* 标题与右侧速览并排：标题只有 4 个字，独占整宽时右侧空掉约 900px。速览三格放进程健康度，与标题
            「运行环境」同一个语义层；不放版本号，那是下面 hero 的主视觉。
            items-end 让两栏底边成一条线：速览比左边的标题+说明高，底对齐时向上探出的那截正好填掉标题上方的
            空带，顶对齐则会在标题下方空一截 */}
        <div className="mb-[64px] flex items-end justify-between gap-[56px]">
          ...
          {data.glance && data.glance.length > 0 && (
            /*
             * 速览格：竖排三行，右对齐 —— 它贴着画布右边缘，取值的右边缘成一条线才像「标注」。
             * 注意：items-end 要在外层与每个格上各写一次 —— 小标签与大数值宽度不同，只在外层对齐的是整格，
             * 格内两行仍会各自左对齐。
             */
        ...
        {/* 版本号主视觉。图标 200px 而不是 168px：右侧三行合计约 220px，168 的图标比文字块矮一截 */}
        <div className="mb-[80px] flex items-center gap-[44px]">
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-[8px]">
            ...
            {/* 字号不写死，由上面的 fitFontSize 按串长算出来走内联 style；text-[130px] 只是兜底，恒被覆盖。
                whitespace-nowrap 而不是 break-word：版本号是一个整体标识，从中间断开比缩小更难读
                （v2.1.0-2- / gc6522ee-dirty 是两个无意义的片段） */}
            <div
        ...
        {/* 环境摘要两列。不做成圆角卡片：状态页与帮助页已经用满了「卡片列表」这个语言，这页再用一遍就会像
            早柚状态。row-gap 比 column-gap 小（52 / 64）：两列之间要读得开，同列上下条目反而该紧一些 */}
          ...
          {data.memory && (
            <div className="flex min-w-0 flex-col gap-[10px]">
              ...
              {/* 百分比大字与「已用 / 总量」小字同基线；flex-wrap 兜底防窄列下溢出。
                  注意：这一串 38px/1.25/-.01em 不是给它自己用（两个子元素各有字号），而是子元素的继承源 ——
                  leading 与 letter-spacing 都是继承属性，删掉的话 pct 的行高会从 57.5px 变成 normal，整格高
                  2px，把页面总高也带偏 */}
        ...
        {data.changes && data.changes.groups.length > 0 && (
          <>
            ...
            {/* 数据来自 CHANGELOG.md（changelog.ts 解析），结构是「分类 + 条目」两层。不做成卡片：这页已经定了
                「无边框信息块」的语言，变更列表再套一层卡片就又向状态页靠回去了。条目那颗小圆点的颜色跟着分类走 */}
              ...
              {data.changes.groups.map((g, gi) => (
                ...
                <div className="min-w-0" key={gi}>
                  ...
                  <ul className="flex list-none flex-col gap-[14px]">
                    {g.items.map((it, ii) => (
                      /* 圆点用 flex-none + mt 手动对齐首行视觉中线：items-center 在条目折行时会把点带到两行
                         之间，看着像挂错了行 */
                      <li
        ...
        {/* 开源信息：两列 × 两行，标签在上、取值在下，四条正好铺满。
            标签上移而不是「标签在左」：那样每列减去标签与间距后取值只剩 442px，装不下 567px 的文档地址，会在
            单词中间断开。也不用三列 —— 每列只有 400px，四条里有三条都超过它。 */}
        {/* border-t-border 而不是 border-border：后者给四边都上色，其余三边虽然宽度为 0 看不见，但计算值会从
            reset 的 currentColor 变掉，逐元素比对时是差异 */}
        <div className="grid grid-cols-2 gap-x-[64px] gap-y-[26px] border-t border-t-border pt-[44px]">
          {data.links.map((l, i) => (
            <div className="flex min-w-0 flex-col gap-[8px] text-[22px]" key={i}>
              ...
              {/* break-all 而不是 break-words：取值是 URL、没有空格可断，而 break-words 只在整词放不下时才断 */}
        ...
        {/* 声明一句隐私边界：本插件只取性能类信息，见 env.ts sysInfo */}
