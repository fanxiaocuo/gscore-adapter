...
export interface RenderOptions {
   ...
   * 超长图分片，交由本体处理
   *
   * 注意：开了它必须走 screenshots() 而不是 screenshot() —— 本体渲染后端在 multiPage 下返回的是 buffer
   * 数组，而 screenshot() 只会把整个数组塞进一个 segment.image，发出去是坏的。
...
export async function render(opts: RenderOptions): Promise<YunzaiSendable | false> {
  ...
  const data = {
     ...
     *
     * 另两个旋钮记在这里，别再重新试一遍：gloss opacity .85→.45 能到 1715KB 但纹理明显发闷；
